...
M1 播放可行性验证已经完成，仓库正在实现 M2 游客浏览到播放的纵向闭环。`BiliAPI` 已接入匿名热门、WBI 签名搜索、视频详情、分 P 和 playurl，并使用脱敏 contract fixture 验证字段与错误边界。App 已具备最小三栏 `NavigationSplitView`，可从真实热门列表进入详情、首个分 P 与系统播放器；状态模型会保留媒体请求头，并隔离快速切换产生的旧结果。搜索入口和完整 M2 错误边界仍待接入，当前版本不适合日常使用或分发。
