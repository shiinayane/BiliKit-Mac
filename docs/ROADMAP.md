...
- 独立、显式运行的真实播放探针；当前 Apple Silicon 开发机上，游客 AVC/AAC 样本已通过起播和双向 seek。

尚不存在：

- 产品级 BiliAPI endpoint、认证、Keychain、播放 UI 纵向接入、弹幕或持久化实现。
- GitHub Actions `macos-15` runner 上的最低版本运行验证；工作流已配置，首次远程结果待推送后确认。
...
- 使用不含凭据的真实 AVC/AAC 样本完成当前 Apple Silicon/macOS 26 环境的 CDN、起播和双向 seek 验证；记录见 [`validation/M1-real-playback-2026-07-21.md`](./validation/M1-real-playback-2026-07-21.md)。

接下来按顺序：

1. 使用多个真实样本补充 CDN fallback、音画同步和失败响应矩阵。
2. 对连续替换与长时间播放做连接数、Task 数和内存增长审计。
3. 在 GitHub Actions `macos-15` runner 复测最低版本；条件允许时覆盖 Intel。
4. 推送 M0/M1 后观察首次远程 CI，只修复可重复的构建或测试问题。
