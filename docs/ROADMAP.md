...
> 状态：M1、M2、M2.5、M3、M4.0–M4.3 已完成；下一步是 M4.4 有界 renderer 与播放 surface 集成。
>
...
1. 进入 M4.4，先固定 renderer 的 lane、容量、丢弃、复用和生命周期契约。
2. 实现 Core Animation/AppKit renderer，并与统一播放时间轴及播放 surface 组合；随后完成 resize、全屏、暂停、seek 和资源归零验证。
