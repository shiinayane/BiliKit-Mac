...
> 状态：M1、M2、M2.5、M3 和 M4.0–M4.4 已完成；下一步是本地持久化纵向切片。
>
...
1. 为首个真实缓存/最近播放调用方确认本地持久化纵向切片契约，再决定是否创建 `BiliPersistence`。
2. 在 M4 总收口时补真实 resize/fullscreen 回归与跨分段 RSS 趋势，不为此预建通用 benchmark framework。
