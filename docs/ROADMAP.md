...
- 首批 `BiliAPI` 游客 endpoint：热门、视频详情、分 P 和 playurl；每个 endpoint 使用独立解码模型，并映射为公开 API 模型或 `PlaybackManifest`。
- 手写脱敏的热门、视频详情、分 P 与 playurl contract fixture，以及 HTML 风控页、API code、字段缺失、输入校验和取消传播测试。
...
- WBI、搜索、认证、Keychain、播放 UI 纵向接入、弹幕或持久化实现。
...
1. 实现 WBI signer 与搜索 contract tests，明确 key 刷新和签名失败重试边界。
2. 将 `BiliAPI` 接入 App 依赖组装，建立热门列表、详情/分 P 和播放器三栏的最小状态模型。
3. 接入最小 `NavigationSplitView`，先完成游客热门 → 详情 → 首个分 P 播放的可操作闭环。
4. 补充搜索入口、加载、空状态、取消、重试和风控错误提示，完成 M2 Gate。
