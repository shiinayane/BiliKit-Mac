...
> 状态：M1、M2、M2.5 已完成；M3 Keychain/请求授权的代码与自动化 Gate 已通过，下一步是签名 Keychain smoke、认证 Application/Feature 和一个个性化闭环。
>
...
- `BiliKitCore` 本地 Swift Package，以及 `BiliModels`、`BiliApplication`、`BiliNetworking`、`BiliAuth`、`BiliAPI`、`BiliPlayback`、`BiliGuestFeature` 七个模块。
...
- 87 个 Package 测试、16 个测试套件和 1 个 App composition 单元测试；原 App 业务测试已迁移到对应 Application/Feature target。
...
- `BiliNetworking.HTTPRequestAuthorizing` 保持无业务语义；具体授权器只允许 `GET https://api.bilibili.com/x/web-interface/nav`，独立拒绝 HTTP、相似主机、CDN、loopback、错误路径/方法、预置 Cookie 和跨主机重定向。
...
- 签名 App 中的真实 Data Protection Keychain 往返、认证 Application/Feature、已登录业务 endpoint、完整 UI 登出、弹幕或业务持久化实现。
...
4. **Keychain 与请求授权（代码与自动化已完成，签名 smoke 待完成）**
...
- 本阶段自动化证据与签名环境边界见 [`validation/M3-keychain-authorization-2026-07-21.md`](./validation/M3-keychain-authorization-2026-07-21.md)。

接下来按顺序：

1. 为 App target 配置最小 Keychain entitlement，在签名本地构建中使用独立测试 service/account 完成真实 Data Protection Keychain add/update/read/delete smoke，并在 teardown 删除；未签名 CI 不伪装此证据。
2. 在 `BiliApplication` 定义非秘密认证状态与用户意图 port，创建有真实调用方的 `BiliAuthFeature`，接入二维码、恢复、重试和完整本地登出。
3. 在历史、收藏或登录推荐中选择一个纵向闭环，让 `BiliAPI` 的该 endpoint 显式请求授权；继续证明游客 endpoint、CDN、loopback 与重定向不携带 Cookie。
