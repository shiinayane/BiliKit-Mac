...
> 状态：M1、M2、M2.5、M3 和 M4.0–M4.4 已完成；M4 scheduler 窄修复、本机完整链路验证与独立红区复审已通过，等待 PR 的 macOS 15/26 CI 后正式关闭。
>
...
M4 总 Gate 结论：本机候选通过，尚未正式关闭。首次红区终审发现 renderer-only probe 绕过 scheduler，且去重 ID 不随三段 cache 淘汰；D1 窄修复现已用八段虚拟时间测试固定相邻重复、seek/generation/reset/identity/disable 语义和三段保留上界。80 events/s 的完整 `repository → DanmakuSession → scheduler → controller → renderer` 链路运行 30 分钟并跨六段，RSS 预热后没有随段数单调增长，stop 后 session、timeline 订阅、controller active count、layer 与 root attachment 归零；修改后的 179 项统一 App Gate 与最终独立红区终审通过。仍须 PR 的 macOS 15/26 CI 通过后才能正式关闭 M4。完整记录见 [`validation/M4-closeout-2026-07-23.md`](./validation/M4-closeout-2026-07-23.md)。
...
1. 提交 M4 scheduler 去重保留窄修复与总收口证据 PR。
2. macOS 15/26 CI 通过后正式关闭 M4，随后以独立生产契约进入 M4.5。

M4 期间不开始复杂导航和视觉精修；本地持久化已降级为未来由搜索历史或其他明确跨重启需求触发的独立切片。更多真实样本与 Intel 覆盖属于兼容性扩展，但发现可重复回归时必须回到对应的 M1/M2 测试层修复。
