...
- Swift 6；App、单元测试和 UI 测试 deployment target 为 macOS 14。
- 模板 App、单元测试 target、UI 测试 target。
- `BiliKitCore` 本地 Swift Package，以及 `BiliModels`、`BiliNetworking`、`BiliPlayback` 三个模块。
...
- 7 个 Package 测试和 1 个 App 单元测试。
- macOS 14 下通过的无签名 `build-for-testing`；构建产物不包含研究文档。

尚不存在：

- BiliAPI endpoint、认证、Keychain、实际 AVPlayer Bridge、弹幕或持久化实现。
- macOS 14 真实运行时的核心播放链路验证。
- 远程 GitHub Actions 运行结果；工作流在提交和推送前只能完成本地静态检查。
...
- 统一 App、测试和 package 的 macOS 14 / Swift 6 配置。
...
- 使用自定义 URL scheme 与 `AVAssetResourceLoaderDelegate` 处理 Range 请求。
- 最小 `AVPlayerView` 宿主，可播放、暂停和双向 seek。
- CDN 候选排序、失败切换、取消和最小诊断事件。
- macOS 14 真实运行测试记录；条件允许时覆盖 Apple Silicon 与 Intel。
...
- macOS 14 与当前 macOS 上完成游客、登录、播放、seek、弹幕和登出 smoke test。
...
1. 提交并推送 M0 后观察首次远程 CI；失败时只修复可重复的构建或测试问题。
2. 为 `BiliNetworking` 补充请求取消与 Range 请求测试。
3. 制作自有 fMP4、SIDX 和 Range fixture。
4. 实现 SIDX parser，并以错误 box、越界和多 reference 样本建立测试。
5. 生成第一版 HLS media playlist，开始 M1 播放 Spike。

完成 M1 Gate 之前，不开始登录、SwiftData、复杂导航和视觉精修。
