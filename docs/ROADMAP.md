...
> 状态：M1、M2、M2.5 已完成；M3 Keychain/请求授权与签名 Data Protection Keychain Gate 已通过，下一步是认证 Application/Feature 和一个个性化闭环。
>
...
- 87 个 Package 测试、16 个测试套件、1 个 App composition 单元测试和 1 个签名 Keychain smoke；原 App 业务测试已迁移到对应 Application/Feature target。
...
- 认证 Application/Feature、已登录业务 endpoint、完整 UI 登出、弹幕或业务持久化实现。
...
1. 在 `BiliApplication` 定义非秘密认证状态与用户意图 port，创建有真实调用方的 `BiliAuthFeature`，接入二维码、恢复、重试和完整本地登出。
2. 在历史、收藏或登录推荐中选择一个纵向闭环，让 `BiliAPI` 的该 endpoint 显式请求授权；继续证明游客 endpoint、CDN、loopback 与重定向不携带 Cookie。
