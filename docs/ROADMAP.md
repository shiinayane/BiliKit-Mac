...
> 状态：M1、M2、M2.5 已完成；M3 认证 Application/Feature 与完整本地登出已完成，下一步是一个个性化纵向闭环和最终实机验收。
>
...
- `BiliKitCore` 本地 Swift Package，以及 `BiliModels`、`BiliApplication`、`BiliNetworking`、`BiliAuth`、`BiliAuthFeature`、`BiliAPI`、`BiliPlayback`、`BiliGuestFeature` 八个模块。
...
- 97 个 Package 测试、18 个测试套件、1 个 App composition 单元测试和 1 个签名 Keychain smoke；原 App 业务测试已迁移到对应 Application/Feature target。
...
- 当前 macOS 已完成账号入口、未登录恢复、现场二维码显示与取消 smoke；没有扫码，也没有输出或保存二维码内容。
- CI 已加入已知二维码 key、Cookie 与 refresh token 模式扫描；`qrcode_key` 进入 URL 日志脱敏集合。

尚不存在：

- 已登录业务 endpoint、弹幕或业务持久化实现；最终扫码登录、重启恢复和界面登出仍需与首个个性化闭环一起实机验收。
...
1. 在历史、收藏或登录推荐中选择一个纵向闭环，让 `BiliAPI` 的该 endpoint 显式请求授权；继续证明游客 endpoint、CDN、loopback 与重定向不携带 Cookie。
2. 在该闭环完成后执行一次真实扫码登录、App 重启恢复、个性化数据读取和界面登出；确认 Keychain 无残留并关闭 M3。
