...
> 状态：M1、M2、M2.5、M3、M4、M4.5、M4.6、M4.7 和 M4.8 已完成；
> Governance v1 已合入。当前产品阶段是 M5，M5.0 的遗留实现仍须按新治理恢复契约、
> 验收和提交，不能视为已关闭。
>
...
- 已有未进入 `origin/main` 的实现候选可以作为恢复输入，但不是可审计的当前事实，
  也不能视为 M5.0 已关闭。恢复工作必须先固定候选 diff，并说明它相对
  [`M5.0-daily-client-state-retention-decision.md`](./development/M5.0-daily-client-state-retention-decision.md)
  revision D2 改变的 Outcome floor、owner 和迁移责任，建立新 revision；不得按
  “语义不变维护”直接关闭。
- 新 revision 必须把用户要求的“返回进去时的样子”翻译为可观察结果，明确热门／搜索
  在 tab 往返与播放返回中的授权矩阵、列表返回表示的 owner，以及稳定布局、内容变化
  与窗口 resize 边界。
...
- 新 revision、实际用户授权、普通 reviewer 的直接验收矩阵、architecture reviewer
  的 owner／迁移检查、red reviewer 的任务与资源生命周期检查、最高适用 App Gate 和
  macOS 15/26 CI 一致后才关闭 M5.0。

#### M5.1：首页个性推荐来源决策与纵向切片

- Outcome floor 是一个可独立发现、语义准确的个性推荐首页：只使用最终选定的一个
...
- 结构 probe 默认无秘密、显式运行且不进入 App 产品路径，只能淘汰不充分候选，不能
  ...
  返回认证／来源决策 Gate；不得向候选请求试带现有 Cookie，也不接入 App token。
- 生产契约必须定义 source identity、auth epoch、每个首页工作集的单 active page
...
- Outcome floor 是从任一来源进入视频 A 后，可以在同一窗口、同一 player host 上经
...
- 点击推荐后先建立最新导航 intent／候选 generation；当前 active playback identity
  与媒体资源何时退休由 production revision 的 commit point 决定。候选不得建立第二
  player host 或第二 active media session。
- M5.2 生产 revision 必须在实现前定义新 identity 的 commit point：提交前候选准备
...
1. 在保留现有 M5.0 遗留实现的前提下建立新 revision，修正 Outcome floor、Feature/App
   owner 迁移和直接 UI 验收矩阵；通过受影响的架构、红区、App Gate 与 CI 后关闭。
2. 为 M5.1 建立 App/Web 首页来源决策 Gate，只选择一个生产来源，再实施有界个性推荐
   首页、自动追加、骨架状态和返回连续性。
