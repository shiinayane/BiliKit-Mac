...
> 状态：M1、M2、M2.5 已完成；M3 安全设计与基础 QR 状态机已完成，成功/过期契约待人工确认。
>
...
- 64 个 Package 测试和 1 个 App composition 单元测试；原 App 业务测试已迁移到对应 Application/Feature target。
...
- `BiliAuth` 已实现认证专用 ephemeral session、重定向拒绝、QR 获取与 `86101` 轮询、取消/旧代次隔离、精确二维码主机校验和内存二维码渲染。
- `BiliAuthProbe` 可显式显示内存二维码并进行脱敏轮询；未知状态失败关闭，尚不解析成功凭据。
- CI 已加入已知二维码 key、Cookie 与 refresh token 模式扫描；`qrcode_key` 进入 URL 日志脱敏集合。

尚不存在：

- 登录成功/过期契约、Keychain 凭据、请求授权、登录 UI、弹幕或业务持久化实现。
...
- 以 10 项全假值测试固定生成、`86101`、取消、网络/HTML、未知状态、精确主机白名单与生成/轮询旧代次隔离；其他业务状态一律失败关闭。
- 把 `qrcode_key` 纳入日志脱敏，并在 macOS 15/26 CI 增加二维码 key、Cookie 和 refresh token 的已知模式扫描。

接下来按顺序：

1. 运行显式人工扫码探针，先确认已扫码未确认状态；每发现一个新状态，只记录状态码/字段结构并补充全假值 fixture，再继续下一轮。
2. 依次确认成功、过期、凭据来源、Cookie 白名单与登录态校验契约；探针只能输出字段名、类型、名称和状态，不输出任何值。
