...
> 状态：M1、M2、M2.5 已完成；M3 Web QR 成功/过期契约 Gate 已通过，下一步实现 Keychain 与请求授权。
>
...
- 70 个 Package 测试和 1 个 App composition 单元测试；原 App 业务测试已迁移到对应 Application/Feature target。
...
- Keychain 凭据、通用 endpoint 请求授权、登录 UI、弹幕或业务持久化实现。
...
1. 实现 Data Protection Keychain generic-password store，以版本化 envelope 原子读写五项 Cookie；不保存 JSON URL、refresh token 或未知 Cookie。
2. 实现 endpoint 级请求授权器，只向精确允许的 `https://api.bilibili.com` 登录 endpoint 添加 Cookie，并覆盖 CDN、loopback 与跨主机重定向负向测试。
3. 凭据必须先通过 nav 登录态校验再提交 Keychain；补齐损坏/失效凭据、重启恢复、完整本地登出与游客回退测试，然后才创建登录 Feature。
