...
1. 创建 `BiliAPI` target，先建立 endpoint、错误和依赖注入边界，不添加空壳模块。
2. 为 `pagelist`、`playurl` 与一个游客列表入口准备自有、脱敏 fixture 和 contract tests。
3. 实现游客列表 → 视频详情 → 分 P → `PlaybackManifest` 的无 UI 纵向链路，并验证取消和旧结果隔离。
4. 在数据链路稳定后接入最小 `NavigationSplitView`，完成 M2 首个可操作闭环。
