...
- 认证、Keychain、播放 UI 纵向接入、弹幕或持久化实现。
...
1. 将 `BiliAPI` 与 `BiliPlayback` 接入 App 依赖组装，建立游客主流程状态模型。
2. 接入最小 `NavigationSplitView`，先完成游客热门 → 详情/分 P → 首个分 P 播放的可操作闭环。
3. 在同一状态边界接入搜索入口，并保留热门与搜索的加载、空结果和错误状态。
4. 补齐 UI 层的取消、重试和风控错误提示，完成 M2 Gate。
