...
> 状态：M1、M2、M2.5、M3、M4.0–M4.3 已完成；M4.3.5 工程治理本地 Gate 已通过，等待远程 macOS 15/26 CI，关闭前不开始 M4.4 生产实现。
>
...
5. **M4.3.5：工程治理 Gate（本地通过，等待远程 CI）**
...
M4.3.5 本地 Gate 结论：通过。风险分级、任务契约、隔离上下文审查、branch/worktree 规则、分层模型路由、统一 static/package/app Gate 和 PR 模板已落地；三路全新上下文只读审查提出的基线降级、Git 授权、diff 范围、工具链选择和两阶段契约问题均已整改。首次统一 App Gate 发现并修复一个既有字幕测试等待竞态，目标测试连续 10 次及完整 152 项 Package 测试最终通过。Luna/Terra/Sol 对同一自包含竞态题均给出正确结论，但单样本 token 数据不足以证明成本下降，后续真实任务继续留痕。远程 macOS 15/26 CI 尚未运行，因此 M4.3.5 尚未完全关闭。完整记录见 [`validation/M4.3.5-engineering-governance-2026-07-22.md`](./validation/M4.3.5-engineering-governance-2026-07-22.md)。
...
- M4.3.5 本地 Gate 已通过：短分支已引入风险分级、任务契约、隔离审查、统一 Gate 命令和 PR 模板；三路不继承主对话的只读审查完成，现等待远程 macOS 15/26 CI 后正式关闭。

接下来按顺序：

1. 提交并推送 M4.3.5 短分支，通过 PR 运行 macOS 15/26 CI；远程通过后正式关闭工程治理 Gate。
2. M4.3.5 关闭后，先由用户确认 spike 契约，再在不可合入分支测量 Core Animation/AppKit renderer 基线，最后确认 M4.4 生产契约。
