...
- App 已接入最小三栏 `NavigationSplitView`，包含热门列表、详情、分 P、系统播放器以及加载、空结果、失败和重试状态。
- 图片模型会把协议相对或 HTTP CDN 地址规范化为 HTTPS；真实热门列表已显示封面。
- 本地真实界面已完成热门 → 详情/首分 P → 播放器起播与暂停验证。

尚不存在：

- 搜索 UI、认证、Keychain、弹幕或持久化实现。
...
1. 在同一状态边界接入搜索入口，并保留搜索的加载、空结果、取消和错误状态。
2. 补齐热门与搜索之间切换、连续选择和窗口退出时的 UI 级回归证据。
3. 完成风控与不支持轨道提示的界面审计，关闭 M2 Gate。
