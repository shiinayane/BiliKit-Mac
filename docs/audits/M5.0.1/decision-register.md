...
| ARCH-003 | 只删除零调用者 `PlayerEngine` protocol/conformance；保留 event stream | 已实施：全仓仅剩具体 `AVPlayerEngine` 与窄 `PlaybackControlling`／`PlaybackTimelineProviding` 调用；request/state/event 移入语义匹配的 `PlayerTypes.swift`；package gate（237 tests，0 known issues）通过 | `PlayerEvent`／events、具体 engine 方法、两个 Application ports 与状态类型均未删除或合并 | 实施完成 |
