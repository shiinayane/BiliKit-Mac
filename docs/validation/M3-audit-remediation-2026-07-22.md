...
>
> 后续更新：提交 `6236753` 已推送，GitHub Actions run `29843186800` 的 macOS 15/26 矩阵通过；M3 已正式关闭。
...
- 提交：`6236753b72da173dcbf8ad832cad61ae087e3525`。
- GitHub Actions：[CI run 29843186800](https://github.com/shiinayane/BiliKit-Mac/actions/runs/29843186800)。
- macOS 15：Package、架构、秘密、工程静态契约、App `build-for-testing` 与 App 单元测试通过。
- macOS 26：同一矩阵通过。
- 手动触发的 macOS 15 真实播放长矩阵未在普通 push 中运行；本轮媒体来源改动已由本记录第 3 节的当前 macOS 真实播放探针覆盖，既有 M1 手动长矩阵证据不被普通 CI 冒充或替代。

据此，M3 的本地、签名受控、真实纵向与当前代码远程矩阵证据均已满足各自 Gate；2026-07-22 正式关闭 M3。
