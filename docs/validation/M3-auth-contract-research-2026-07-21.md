...
> 结论：本文先记录实现前的最小现场验证，后附同日状态机阶段确认的成功、等待确认与
> 过期结构。它是 2026-07-21 的未公开 Web 协议观察，不是长期服务端契约。
...
- `86101`：未扫码；
- `86090`：已扫码，等待手机确认；
- `0`：成功候选，必须再经精确 nav 请求确认登录态；
- `86038`：二维码过期。

成功候选的结构包含 `refresh_token`、URL 和 Cookie，但生产实现只从响应 Cookie 中接受
固定五项，先请求 `https://api.bilibili.com/x/web-interface/nav` 验证 `isLogin=true`，
再允许持久化。探针只记录字段名、Cookie 名和属性，不输出值、完整 URL、二维码 key 或
响应 body。未知状态、HTML 风控页、异常来源和并发旧 generation 默认失败关闭。

状态机使用独立 ephemeral session、拒绝重定向，并以本地总时限和最大轮询次数限制资源。
二维码只在内存渲染；取消、过期或新 generation 会清除旧 challenge。以上状态仍可能随
远端未公开协议漂移，出现新状态时必须重新做最小脱敏观察。
