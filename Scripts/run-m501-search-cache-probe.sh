...
cleanup() {
    ...
    if [[ -n "$probe_directory"
        && "$probe_directory" == /tmp/BiliKitMac-m501-search-cache.*
...
printf '%s' '请输入一个公开、非敏感的搜索词：' >&2
IFS= read -rs probe_query
printf '\n' >&2
if [[ -z "${probe_query//[[:space:]]/}" || ${#probe_query} -gt 100 ]]; then
    print -u2 '搜索词为空或过长。'
    exit 2
fi

probe_directory=$(mktemp -d /tmp/BiliKitMac-m501-search-cache.XXXXXX)
...
result_bundle="$probe_directory/M501SearchCache.xcresult"
/usr/bin/plutil -create xml1 "$input_file"
/usr/bin/plutil -insert query -string "$probe_query" "$input_file"
...
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
    ...
    -only-testing:BiliKitMacTests/M501SearchCacheProbeTests/testRepeatedAnonymousSearchCacheBehaviorWhenExplicitlyConfigured \
    >> "$test_log" 2>&1
probe_status=$?
set -e
...
if [[ -d "$result_bundle" ]]; then
    DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
        ...
        --path "$result_bundle" \
        --test-id 'M501SearchCacheProbeTests/testRepeatedAnonymousSearchCacheBehaviorWhenExplicitlyConfigured()' \
        >> "$test_log" 2>&1 || true
fi

if [[ $probe_status -ne 0 ]]; then
    print -u2 '搜索缓存探针失败或超时；原始输出已清理。'
    exit $probe_status
fi

summary=$(rg -m 1 -o \
    'm501-search-cache responses=2 status-first=[0-9]+ status-second=[0-9]+ cache-first=[a-z-]+ cache-second=[a-z-]+ bytes-first=[0-9]+ bytes-second=[0-9]+ conditional-first=[01] conditional-second=[01] etag-first=[01] etag-second=[01] last-modified-first=[01] last-modified-second=[01] cleanup=complete' \
    "$test_log" || true)
if [[ -z "$summary" ]]; then
...
print -r -- "$summary"
print '探针完成；搜索词、URL、结果正文、响应和原始产物均未保留。'
