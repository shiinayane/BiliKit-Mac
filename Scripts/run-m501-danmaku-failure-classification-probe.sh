...
if ((segment_index < 1 || segment_index > 10000)); then
    print -u2 '分段序号超出允许范围。'
    exit 2
fi
...
cleanup() {
    ...
    if [[ -n "$probe_directory"
        && "$probe_directory" == /tmp/BiliKitMac-m501-danmaku-classify.*
...
probe_directory=$(mktemp -d /tmp/BiliKitMac-m501-danmaku-classify.XXXXXX)
...
result_bundle="$probe_directory/M501DanmakuClassification.xcresult"
/usr/bin/plutil -create xml1 "$input_file"
/usr/bin/plutil -insert bvid -string "$probe_bvid" "$input_file"
/usr/bin/plutil -insert segment -string "$segment_index" "$input_file"
...
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
    ...
    -only-testing:BiliKitMacTests/M501DanmakuWorkingSetProbeTests/testRealSegmentFailureClassificationWhenExplicitlyConfigured \
    >> "$test_log" 2>&1
probe_status=$?
set -e
...
if [[ -d "$result_bundle" ]]; then
    DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
        ...
        --path "$result_bundle" \
        --test-id 'M501DanmakuWorkingSetProbeTests/testRealSegmentFailureClassificationWhenExplicitlyConfigured()' \
        >> "$test_log" 2>&1 || true
fi

if [[ $probe_status -ne 0 ]]; then
    print -u2 '失败分类探针失败或超时；原始输出已清理。'
    exit $probe_status
fi

summary=$(rg -m 1 -o \
    'm501-danmaku-failure-classification segment=[0-9]+ duration-seconds=[0-9]+ last-segment=[0-9]+ within-duration=[01] result=[a-z-]+ http=[0-9]+ content=[a-z-]+ body-bytes=[0-9]+ wire=[a-z-]+ elements=[0-9]+ unsupported=[0-9]+ blank=[0-9]+ missing-id=[0-9]+ long-id=[0-9]+ time=[0-9]+ long-text=[0-9]+ color=[0-9]+ event-over=[01] text-over=[01] cleanup=complete' \
    "$test_log" || true)
if [[ -z "$summary" ]]; then
    print -u2 '探针没有产生完整的脱敏分类；原始输出已清理。'
    exit 1
fi

print -r -- "$summary"
print '分类完成；BVID、CID、URL、正文、响应和原始产物均未保留。'
