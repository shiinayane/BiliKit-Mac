...
cleanup() {
    ...
    if [[ -n "$probe_directory"
        && "$probe_directory" == /tmp/BiliKitMac-authenticated-playback-probe.*
...
probe_directory=$(mktemp -d /tmp/BiliKitMac-authenticated-playback-probe.XXXXXX)
...
result_bundle="$probe_directory/AuthenticatedPlayback.xcresult"
/usr/bin/plutil -create xml1 "$input_file"
/usr/bin/plutil -insert select-popular-first -string true "$input_file"
...
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
    ...
    -only-testing:BiliKitMacTests/AuthenticatedPlaybackProbeTests/testAuthenticatedPlaybackQualityWhenExplicitlyConfigured \
    >> "$test_log" 2>&1
probe_status=$?
set -e
...
if [[ -d "$result_bundle" ]]; then
    DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
        ...
        --path "$result_bundle" \
        --test-id 'AuthenticatedPlaybackProbeTests/testAuthenticatedPlaybackQualityWhenExplicitlyConfigured()' \
        >> "$test_log" 2>&1 || true
fi
...
summary=$(rg -m 1 -o \
    'authenticated-playback-quality anonymous-http=2xx anonymous-business=0 anonymous-avc=[0-9]+ anonymous-aac=[0-9]+ anonymous-quality=\[[0-9, ]+\] anonymous-heights=\[[0-9, ]+\] anonymous-max-height=[0-9]+ authenticated-http=2xx authenticated-business=0 authenticated-avc=[0-9]+ authenticated-aac=[0-9]+ authenticated-quality=\[[0-9, ]+\] authenticated-heights=\[[0-9, ]+\] authenticated-max-height=[0-9]+ higher=(true|false) cookie-forwarded=false' \
    "$test_log" || true)
if [[ $probe_status -ne 0 || -z "$summary" ]]; then
    stage_summary=$(rg -o \
        'authenticated-playback-stage stage=[a-z-]+' \
